...
def get_snapshots(user_id):
    ...
    for snapshot in result:
        snapshot['link'] = f"http://{gui_host}:{gui_port}/users/{user_id}/snapshots/{snapshot['snapshot_id']}"
...
def get_snapshot(user_id, snapshot_id):
    request = f'http://{api_host}:{api_port}/users/{user_id}/snapshots/{snapshot_id}'
    result, error = send_and_handle_request(request)
    if error:
        return result
    results_dict = {}
    for res in result['result_names']:
        request = f'http://{api_host}:{api_port}/users/{user_id}/snapshots/{snapshot_id}/{res}'
        result, error = send_and_handle_request(request)
        if error:
            return result
        results_dict[res] = result[res]
    return render_template('snapshot.html', results=results_dict, snapshot_id=snapshot_id)
