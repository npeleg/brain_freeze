...
def get_parsers_from_server(host, port):
    ...
    if r.status_code != 200:
        logger.error(f"could not get parsers from server due to error."
                     f"Got {r.status_code} status code")
        return None
...
def send_to_server(host, port, data, user_id):
    if user_id is None:
        ...
        r = requests.post(f'http://{host}:{port}/snapshots/{user_id}',
                          data=data)
    response = r.json()
    if r.status_code != 201:
        logger.error(f"server did not accept data due to error."
                     f"Got {r.status_code} status code")
        return False
...
def upload_sample(host, port, path):
    ...
    user_message = protocol.init_protocol_user(reader.user.user_id,
                                               reader.user.username,
                                               reader.user.birthday,
                                               reader.user.gender)
    ...
    for snapshot in reader:
        ...
        try:
            ...
            partial_snapshot = protocol.build_partial_snapshot(snapshot,
                                                               config)
            serialized_snapshot_message = protocol.serialize(partial_snapshot)
            sent = send_to_server(host, port, serialized_snapshot_message,
