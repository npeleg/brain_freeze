...
def load_parsers(parsers_dir, parsers_dict):
    """ Dynamically imports all the parsers in parsers_dir
    and inserts them to a dictionary"""
    ...
    for path in abs_parsers_dir.iterdir():
        if path.name.startswith('_') or not path.suffix == ".py":
            continue
        module = importlib.import_module(f'{abs_parsers_dir.name}.{path.stem}',
                                         package=abs_parsers_dir.name)
        parse_func = getattr(module, 'parse')
        parsers_dict[module.parse.name] = parse_func


def wrap_parser(parser_name, parser_func, mq):
    """ returns a function that parses data using parser_func
    and publishes the result to the parser_name topic"""
